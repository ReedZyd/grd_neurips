...
def RRD(args):
    ...
    class RandomizedReturnDecomposition(basis_alg_class):
        ...
        def create_inputs(self):
            ...
            self.rrd_raw_obs_ph = tf.compat.v1.placeholder(tf.float32, [None, self.args.rrd_sample_size]+self.args.obs_dims)
            self.rrd_raw_obs_next_ph = tf.compat.v1.placeholder(tf.float32, [None, self.args.rrd_sample_size]+self.args.obs_dims)
            self.rrd_acts_ph = tf.compat.v1.placeholder(tf.float32, [None, self.args.rrd_sample_size]+self.args.acts_dims)
            self.rrd_rews_ph = tf.compat.v1.placeholder(tf.float32, [None, 1])
            if self.args.rrd_bias_correction:
                self.rrd_var_coef_ph = tf.compat.v1.placeholder(tf.float32, [None, 1])
        ...
        def create_network(self):
            ...
            def mlp_rrd(rrd_obs_ph, rrd_acts_ph, rrd_obs_next_ph):
                ...
                with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                    r_dense1 = tf.compat.v1.layers.dense(rrd_state_ph, 256, activation=tf.nn.relu, name='r_dense1')
            ...
            def conv_rrd(rrd_obs_ph, rrd_acts_ph, rrd_obs_next_ph):
                ...
                with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                    r_conv1 = tf.compat.v1.layers.conv2d(rrd_state_ph, 32, 8, 4, 'same', activation=tf.nn.relu, name='r_conv1')
                    r_conv2 = tf.compat.v1.layers.conv2d(r_conv1, 64, 4, 2, 'same', activation=tf.nn.relu, name='r_conv2')
                    r_conv3 = tf.compat.v1.layers.conv2d(r_conv2, 64, 3, 1, 'same', activation=tf.nn.relu, name='r_conv3')
                    r_conv3_flat = tf.compat.v1.layers.flatten(r_conv3)

                    r_dense_act = tf.compat.v1.layers.dense(r_conv3_flat, 512, activation=tf.nn.relu, name='r_dense_act')
                    r_act = tf.compat.v1.layers.dense(r_dense_act, self.acts_num, name='r_act')
                    r = tf.reduce_sum(input_tensor=r_act*rrd_acts_ph, axis=-1, keepdims=True)
                if flatten:
                    r = tf.reshape(r, [-1, self.args.rrd_sample_size, 1])
                return r

            reward_net = mlp_rrd if len(self.args.obs_dims)==1 else conv_rrd
            with tf.compat.v1.variable_scope('rrd'):
                self.rrd_rews_pred = reward_net(self.rrd_obs_ph, self.rrd_acts_ph, self.rrd_obs_next_ph)
                self.rrd = tf.reduce_mean(input_tensor=self.rrd_rews_pred, axis=1)
            with tf.compat.v1.variable_scope('rrd', reuse=True):
        ...
        def create_operators(self):
            ...
            self.r_loss = tf.reduce_mean(input_tensor=tf.square(self.rrd-self.rrd_rews_ph))
            if self.args.rrd_bias_correction:
                assert self.args.rrd_sample_size>1
                n = self.args.rrd_sample_size
                self.r_var_single = tf.reduce_sum(input_tensor=tf.square(self.rrd_rews_pred-tf.reduce_mean(input_tensor=self.rrd_rews_pred, axis=1, keepdims=True)), axis=1) / (n-1)
                self.r_var = tf.reduce_mean(input_tensor=self.r_var_single*self.rrd_var_coef_ph/n)
