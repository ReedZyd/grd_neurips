...
class DDPG(Base):
    ...
    def create_network(self):
        def mlp_policy(obs_ph):
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                pi_dense1 = tf.compat.v1.layers.dense(obs_ph, 400, activation=tf.nn.relu, name='pi_dense1')
                pi_dense2 = tf.compat.v1.layers.dense(pi_dense1, 300, activation=tf.nn.relu, name='pi_dense2')
                pi = tf.compat.v1.layers.dense(pi_dense2, self.args.acts_dims[0], activation=tf.nn.tanh, name='pi')
            return pi

        def mlp_value(obs_ph, acts_ph):
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                q_dense1 = tf.compat.v1.layers.dense(obs_ph, 400, activation=tf.nn.relu, name='q_dense1')
                q_dense2 = tf.compat.v1.layers.dense(tf.concat([q_dense1, acts_ph], axis=1), 300, activation=tf.nn.relu, name='q_dense2')
        ...
        with tf.compat.v1.variable_scope('main'):
            ...
            with tf.compat.v1.variable_scope('value', regularizer=tf.keras.regularizers.l2(0.5 * (self.args.q_reg))):
    ...
    def feed_dict(self, batch):
        return {
            ...
            self.dense_rews_ph: batch['dense_reward'],
        }
