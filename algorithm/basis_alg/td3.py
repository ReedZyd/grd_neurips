...
class TD3(DDPG):
    ...
    def create_inputs(self):
        ...
        self.target_noise_ph = tf.compat.v1.placeholder(tf.float32, [None]+self.args.acts_dims)
    ...
    def create_network(self):
        ...
        with tf.compat.v1.variable_scope('main'):
            ...
            with tf.compat.v1.variable_scope('value_1'):
                self.q_1 = mlp_value(self.obs_ph, self.acts_ph)
            with tf.compat.v1.variable_scope('value_2'):
                self.q_2 = mlp_value(self.obs_ph, self.acts_ph)
            with tf.compat.v1.variable_scope('value_1', reuse=True):
                self.q_pi = mlp_value(self.obs_ph, self.pi)
            with tf.compat.v1.variable_scope('value_2', reuse=True):
    ...
    def create_operators(self):
        ...
        self.q_loss = tf.reduce_mean(input_tensor=tf.square(self.q_1-target)) + tf.reduce_mean(input_tensor=tf.square(self.q_2-target))
