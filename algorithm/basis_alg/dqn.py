    ...
    def create_network(self):
        def mlp_value(obs_ph):
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                q_dense1 = tf.compat.v1.layers.dense(obs_ph, 256, activation=tf.nn.relu, name='q_dense1')
                q_dense2 = tf.compat.v1.layers.dense(q_dense1, 256, activation=tf.nn.relu, name='q_dense2')
                q = tf.compat.v1.layers.dense(q_dense2, self.acts_num, name='q')
            return q

        def conv_value(obs_ph):
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                q_conv1 = tf.compat.v1.layers.conv2d(obs_ph, 32, 8, 4, 'same', activation=tf.nn.relu, name='q_conv1')
                q_conv2 = tf.compat.v1.layers.conv2d(q_conv1, 64, 4, 2, 'same', activation=tf.nn.relu, name='q_conv2')
                q_conv3 = tf.compat.v1.layers.conv2d(q_conv2, 64, 3, 1, 'same', activation=tf.nn.relu, name='q_conv3')
                q_conv3_flat = tf.compat.v1.layers.flatten(q_conv3)

                q_dense_act = tf.compat.v1.layers.dense(q_conv3_flat, 512, activation=tf.nn.relu, name='q_dense_act')
                q_act = tf.compat.v1.layers.dense(q_dense_act, self.acts_num, name='q_act')

                if self.args.dueling:
                    q_dense_base = tf.compat.v1.layers.dense(q_conv3_flat, 512, activation=tf.nn.relu, name='q_dense_base')
                    q_base = tf.compat.v1.layers.dense(q_dense_base, 1, name='q_base')
                    q = q_base + q_act - tf.reduce_mean(input_tensor=q_act, axis=1, keepdims=True)
                else:
                    q = q_act
            return q
        ...
        with tf.compat.v1.variable_scope('main'):
            with tf.compat.v1.variable_scope('value'):
                ...
                self.q_pi = tf.reduce_max(input_tensor=self.q, axis=1, keepdims=True)
            if self.args.double:
                with tf.compat.v1.variable_scope('value', reuse=True):
                    self.q_next = value_net(self.obs_next_ph)
                    self.pi_next = tf.one_hot(tf.argmax(input=self.q_next, axis=1), self.acts_num, dtype=tf.float32)

        with tf.compat.v1.variable_scope('target'):
            with tf.compat.v1.variable_scope('value'):
                if self.args.double:
                    self.q_t = tf.reduce_sum(input_tensor=value_net(self.obs_next_ph)*self.pi_next, axis=1, keepdims=True)
                else:
                    self.q_t = tf.reduce_max(input_tensor=value_net(self.obs_next_ph), axis=1, keepdims=True)

    def create_operators(self):
        target = tf.stop_gradient(self.rews_ph+(1.0-self.done_ph)*self.args.gamma*self.q_t)
        self.q_acts = tf.reduce_sum(input_tensor=self.q*self.acts_ph, axis=1, keepdims=True)
        self.q_loss = tf.compat.v1.losses.huber_loss(target, self.q_acts)
        if self.args.optimizer=='adam':
            self.q_optimizer = tf.compat.v1.train.AdamOptimizer(self.args.q_lr, epsilon=self.args.Adam_eps)
        elif self.args.optimizer=='rmsprop':
            self.q_optimizer = tf.compat.v1.train.RMSPropOptimizer(self.args.q_lr, decay=self.args.RMSProp_decay, epsilon=self.args.RMSProp_eps)
