...
class SAC(DDPG):
    ...
    def create_inputs(self):
        ...
        self.pi_noise_ph = tf.compat.v1.placeholder(tf.float32, [None]+self.args.acts_dims)
        self.pi_next_noise_ph = tf.compat.v1.placeholder(tf.float32, [None]+self.args.acts_dims)
    ...
    def create_network(self):
        def mlp_policy(obs_ph):
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                ...
                pi_mean_logstd = tf.compat.v1.layers.dense(pi_dense2, self.args.acts_dims[0]*2, name='pi')
        ...
        def mlp_q_value(obs_ph, acts_ph):
            ...
            with tf.compat.v1.variable_scope('net', initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
                ...
                q = tf.compat.v1.layers.dense(q_dense2, 1, name='q')
            return q

        with tf.compat.v1.variable_scope('main'):
            with tf.compat.v1.variable_scope('alpha'):
            ...
            def get_pi_log_p(pi, pi_sample, pi_act):
                return pi.log_p(pi_sample) - tf.reduce_sum(input_tensor=tf.math.log(1-tf.square(pi_act)+1e-6), axis=-1, keepdims=True)
            ...
            with tf.compat.v1.variable_scope('q_value_1'):
                self.q_1 = mlp_q_value(self.obs_ph, self.acts_ph)
            with tf.compat.v1.variable_scope('q_value_2'):
                self.q_2 = mlp_q_value(self.obs_ph, self.acts_ph)
            with tf.compat.v1.variable_scope('q_value_1', reuse=True):
                self.q_pi_1 = mlp_q_value(self.obs_ph, self.pi_act)
            with tf.compat.v1.variable_scope('q_value_2', reuse=True):
    ...
    def create_operators(self):
        self.pi_loss = tf.reduce_mean(input_tensor=-self.q_pi)
        ...
        if self.args.alpha_lr>0:
            self.alpha_loss = -self.log_alpha*tf.stop_gradient(tf.reduce_mean(input_tensor=self.pi_log_p) - np.prod(self.args.acts_dims))
            self.alpha_optimizer = tf.compat.v1.train.AdamOptimizer(self.args.alpha_lr)
        ...
        self.q_loss = tf.reduce_mean(input_tensor=tf.square(self.q_1-q_target)) + tf.reduce_mean(input_tensor=tf.square(self.q_2-q_target))
