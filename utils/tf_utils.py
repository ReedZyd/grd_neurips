...
def get_reg_loss(scope_name):
    return tf.reduce_mean(input_tensor=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, scope=scope_name))
...
class RandomNormal:
    ...
    def log_p(self, x):
        return tf.reduce_sum(
            input_tensor=-0.5*np.log(2.0*np.pi)-self.logstd-0.5*tf.square((x-self.mean)/self.std),
            axis=-1, keepdims=True
        )

    def entropy(self):
        return tf.reduce_sum(input_tensor=self.logstd+0.5*np.log(2.0*np.pi*np.e), axis=-1, keepdims=True)

    def kl(self, other):
        return tf.reduce_sum(
            input_tensor=-0.5+other.logstd-self.logstd
